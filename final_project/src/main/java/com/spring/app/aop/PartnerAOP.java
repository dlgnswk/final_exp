...
public class PartnerAOP {
	...
	*/
	...
	@Pointcut("execution(public * com.spring.app.wh..*Controller.requiredLogin_*(..))")
	...
	public void loginCheck(JoinPoint joinpoint) { // 로그인 유무 검사를 하는 메소드 작성하기
		...
		if(session.getAttribute("loginhost") == null) {
	...
	@Pointcut("execution(public * com.spring.app.wh..*Controller.pointPlus_*(..))")
	...
	*/
	...
	/*
	@Pointcut("execution(String com.spring.app.employees.controller.ScheduleController.schedulemanager_*(..))")
	public void empmanager() {} 
	
	@Autowired
	private ScheduleService empservice;
	
	// == Around Advice(공통관심사, 보조업무)를 구현한다.
    // - 직원관리와 관련된 주업무를 실행하는 데 있어서 권한이 있는지(로그인 되어진 사용자의 gradelevel 값이 10)를 알아보는것을 보조업무로 보겠다. 
    // - 인사관리 페이지에 접속한 이후에, 인사관리 페이지에 접속한 페이지URL, 사용자ID, 접속IP주소, 접속시간을 기록으로 DB의 tbl_empManger_accessTime 테이블에 insert 하도록 한다.
	@Around("empmanager()")
	public String checkAuthority(ProceedingJoinPoint joinPoint) {
		// Around Advice 에서는 ProceedingJoinPoint joinPoint 가포인트컷 되어진 주업무의 메소드이다. 
		...
		if(loginuser == null || loginuser.getGradelevel() < 10) {
			...
			try {
				...
				// ~~~~ 확인용 viewPage => emp/empList.tiles2
				// ~~~~ 확인용 viewPage => emp/chart.tiles2
