...
public class FileManager {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		newFileName += System.nanoTime(); // 시간
		newFileName += fileExt;
		// newFileName 은 예를 들면, 2023112409291535243254235235234.png
		...
		if(!dir.exists()) {
			...
			dir.mkdirs(); // .mkdir()은 서브폴더를 못만들기 때문에 mkdirs()를 쓴다.
		}
		...
		// pathname 은 예를 들면, C:\Users\82105\Desktop\ssit\java_file\NCS\workspace_spring_framework\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\board\resources\files\2023112409291535243254235235234.png 이다.
	...
	public boolean doFileDownload(String fileName, String originalFilename, String path, HttpServletResponse response) {
        ...
        // 해당경로에 \ 를 더하고 파일명을 더한 경로까지 나타내어준 파일명(문자열)을 만든다. 
		// path name은 예를 들면, C:\Users\82105\Desktop\ssit\java_file\NCS\workspace_spring_framework\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\board\resources\files\20231124124858547167665583200.jpg 이다.
		
		try {
		
			if(originalFilename == null || "".equals(originalFilename)) {
				originalFilename = fileName; // org파일이름이 없으면 fileName 그대로 받는다.
			}
			// originalFilename = new String(originalFilename.getBytes("EUC-KR"),"8859_1"); // 한글
			// 또는
		...
		return false;
	} // end of public String doFileDownload(String fileName, String originalFilename, String path, HttpServletResponse response)---------
